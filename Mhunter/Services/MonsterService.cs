...
namespace Mhunter.Services
{
    public class MonsterService
    {
        ...
        public virtual async Task<List<Monster>> GetAllMonsters()
        {
            return await _http.GetFromJsonAsync<List<Monster>>("https://mhw-db.com/monsters");
        }

        public virtual async Task<Monster?> GetMonsterById(int id)
        {
            return await _http.GetFromJsonAsync<Monster?>($"https://mhw-db.com/monsters/{id}");
        }

        public virtual async Task<Monster?> GetMonsterByName(string name)
        {
        ...
        public virtual async Task<Monster?> GetRandomMonster()
        {
            var monsters = await GetAllMonsters();
            var rnd = new Random();
